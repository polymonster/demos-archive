...
bool CModelMD3::LoadWeapon(LPSTR strPath, LPSTR strModel)
{
	...
	char twatty[25] = {0};
	strcpy(twatty,strModel);
	sprintf(strWeaponMuzzle, "%s%s.bmp", "data/textures/particles/flares/f_",twatty);
	m_Weapon.muzzleId = CTextureMan::getInstance()->addSkin(strWeaponMuzzle,1);
...
void CModelMD3::LoadModelTextures(t3DModel *pModel, LPSTR strPath)
{
	...
	for(int i = 0; i < pModel->numOfMaterials; i++)
	{
		...
		if(strlen(pModel->pMaterials[i].strFile) > 0)
		{
			...
			pModel->pMaterials[i].texureId = CTextureMan::getInstance()->addSkin(strFullPath,0);
...
void CModelMD3::UpdateModel(t3DModel *pModel)
{
	...
	{
		if((g_partCounter == 3) && ((pModel->currentAnim == TORSO_ATTACK) || (pModel->currentAnim == TORSO_ATTACK2)))
		{
			m_Upper.currentAnim = TORSO_STAND;
...
void CModelMD3::DrawModel(float xrot)
{
...
void CModelMD3::DrawLink(t3DModel *pModel,bool doxrot,float xrot)
{
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
				...
				if(g_partCounter == weaponPartID)
				{
					gunPos.x = vPosition.x;
					gunPos.y = vPosition.y - 3;
					gunPos.z = vPosition.z;

					getPointFromNewMatrix(gunPos,finalMatrix,10,30,-10,1,0,0);
					bulletStartPos = gunPos;

					glPushMatrix();
					glTranslated(12,0,2);
					glScaled(10,10,10);
					glGetFloatv(GL_MODELVIEW_MATRIX,muzzleMatrix);
					glPopMatrix();
				}
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
				...
				g_partCounter++;
				if(g_partCounter == 1) DrawLink(pLink,1,xrot);
				else DrawLink(pLink,0,xrot);
...
void CModelMD3::SetCurrentTime(t3DModel *pModel)
{
	...
	float time = CTimer::getInstance()->time() / 0.001f;
...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
		...
		if(pObject->bHasTexture)
		{
			...
			CTextureMan::getInstance()->bindSkin(pModel->pMaterials[pObject->materialID].texureId);
			...
			for(int j = 0; j < pObject->numOfFaces; j++)
			{
				...
				for(int whichVertex = 0; whichVertex < 3; whichVertex++)
				{
					...
					if(pObject->pTexVerts) 
					{
						...
						glMultiTexCoord2f(GL_TEXTURE1_ARB,pObject->pTexVerts[ index ].x, 
...
void CModelMD3::SetNextTorsoAnimation(LPSTR strAnimation)
{
	// Go through all of the animations in this model
	for(int i = 0; i < m_Upper.numOfAnimations; i++)
	{
