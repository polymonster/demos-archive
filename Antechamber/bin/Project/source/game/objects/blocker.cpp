...
Blocker::~Blocker()
{
	g_sound_mgr_p->stopLoop(g_sound_id.gate_motion);
}
...
void Blocker::execute( void )
{
	...
	if(m_inmotion)
	{
		if(m_channelID == -1)
		{
			m_channelID = g_sound_mgr_p->playLoop(g_sound_id.gate_motion);
		}
	}
	else
	{
		...
		{
			g_sound_mgr_p->stopLoop(g_sound_id.gate_motion);
			g_sound_mgr_p->play(g_sound_id.gate_stop,55);
			m_channelID = -1;
		}
	}
...
void Blocker::activate( void )
{
	if(m_open)
	{
		g_sound_mgr_p->play(g_sound_id.gate_close,25);
	}
	else
	{
		g_sound_mgr_p->play(g_sound_id.gate_open,25);
	}
