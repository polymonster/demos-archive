...
Link *Linklist::get_obj( int link_handle )
{
	if( link_handle < 0 )
	{
		dbgmsg( "Find link error! Link handle below zero.\n", link_handle );
		return NULL;
	}
...
void Linklist::remove( int &link_handle, bool deleteobj )
{
	if( link_handle < 0 )
	{
		dbgmsg( "Delete link error! Link handle below zero.\n", link_handle );
		return;
	}
	Link *link_p = m_first_p;
	while( link_p && link_handle != link_p->get_handle() )
	...
	if( !link_p )
	{
		dbgmsg( "Delete link error! Link handle %d not found or invalid.\n", link_handle );
		return;
	}
