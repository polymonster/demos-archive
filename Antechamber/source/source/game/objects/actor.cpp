...
void Actor::jump()
{
	...
	{
		g_sound_mgr_p->play(S_DOUBLE_JUMP);
		m_anim_state = ANIM_STATE_TAKE_OFF;
		m_velocity_y = JUMP_VELOCITY;
		m_in_air = false;
	}
	...
	if(m_dead)
	{
		...
		if(m_anim_state == ANIM_STATE_FINISHED)
		{
			...
			if(m_respawn_timer <= 0)
			{
				...
				g_sound_mgr_p->play(S_RESPAWN,15);
...
void Actor::render( void )
{
	if(m_user_control)
	{
		...
		{
			char output[64];
			g_mini_font_p->print_2d_text( "[PLAYER 1]", 8, 68, 1.0f, 0xffe0e0e0 );
			...
			g_mini_font_p->print_2d_text( output, 8, 88, 1.0f, 0xffe0e000 );
		...
		{
			char output[64];
			g_mini_font_p->print_2d_text( "[PLAYER 2]", 580, 68, 1.0f, 0xffe0e0e0 );
			sprintf( output, "%cLives%c %d", S_COL_GREY, 1, m_lives );
			g_mini_font_p->print_2d_text( output, 580, 78, 1.0f, 0xffe0e000 );
			sprintf( output, "%cScore%c %d", S_COL_GREY, 1, m_score );
			g_mini_font_p->print_2d_text( output, 580, 88, 1.0f, 0xffe0e000 );

			if(m_display_tooltip)
			{
				char text[64];
				sprintf( text, "[Press Keyboard[NUMPAD 8] or Xbox Pad[X] to Interact]" );
				g_mini_font_p->print_2d_text( text, 320-g_mini_font_p->get_text_width( text )*0.5f, 78, 1.0f, 0xffe0c00a );
			}
		}
	}
