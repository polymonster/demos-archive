...
Turret::Turret(vector2 position, float fire_rate, int type, int projectile_type, int rotation_range,  char *name_p) :
	...
	Quad(PATH_SPRITES,"barrel",position.x,position.y,3,8,0xffffffff)
{
	set_priority( RENDER_FRONT - 5.0f );
	vector2 baseOffsets;
...
int Turret::shoot( vector2 *pos, vector2 *dir )
{
	if(m_fire_timer <= 0)
	{
		...
		*dir = m_aim_vector;
		...
		{
			m_cooldown_timer = 4;
			m_sound_channel = g_sound_mgr_p->playLoop(S_BEAM,15);
		}
		else if(m_projType == 1) g_sound_mgr_p->play(S_MISSILE);
		else g_sound_mgr_p->play(S_BULLET);
