...
{
    /// <remarks>
    /// Creates and manages the graphics for Minkowski Sums.
    /// </remarks>
    public class DisplayMinkowskiSum : DisplayObject
    {
        MinkowskiSum sum;
        ...
        public DisplayMinkowskiSum(MinkowskiSum s, GraphicsDeviceManager graphics, bool initShape, bool initVertexBuffer)
        {
        ...
        void initializeShape()
        {
            ...
            {//per column
                ...
                {//per row
                    ...
                    sum.getExtremePoint(ref dir, sum.collisionMargin, out extreme);
                    bodyPoints.Add(extreme - sum.centerPosition);
                }
            }
            sum.getExtremePoint(ref Toolbox.upVector, sum.collisionMargin, out extreme);
            bodyPoints.Add(extreme - sum.centerPosition);
            sum.getExtremePoint(ref Toolbox.downVector, sum.collisionMargin, out extreme);
            bodyPoints.Add(extreme - sum.centerPosition);
            ...
            Quaternion inverseOrientation = Quaternion.Conjugate(sum.orientationQuaternion);
        ...
        public override VertexPositionNormalTexture[] getVertices()
        {
            ...
            for (int i = 0; i < hullTriangleVertices.Count; i += 3)
            {
                ...
                toReturn[i] = new VertexPositionNormalTexture(hullTriangleVertices[i] - sum.centerPosition, normal, textureCoords[0]);
                toReturn[i + 1] = new VertexPositionNormalTexture(hullTriangleVertices[i + 1] - sum.centerPosition, normal, textureCoords[1]);
                toReturn[i + 2] = new VertexPositionNormalTexture(hullTriangleVertices[i + 2] - sum.centerPosition, normal, textureCoords[2]);
            }
            Quaternion inverseOrientation = Quaternion.Conjugate(sum.orientationQuaternion);
